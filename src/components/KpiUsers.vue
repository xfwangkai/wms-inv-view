...
<script>
  ...
  import {fetchKpiUsers, saveKpiUsers} from "@/api/kpi-api"
  ...
  export default {
    ...
    methods: {
      commitDatas(){
        saveKpiUsers({data: this.$refs.hotTableComponent.hotInstance.getSourceData(), delData: this.deleteRowsList}).then(res => {
          this.fetchData()
        })
      },
      fetchData(){
        fetchKpiUsers({}).then(res => {
